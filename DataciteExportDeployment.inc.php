...
class DataciteExportDeployment extends PKPImportExportDeployment {
	...
	public function getNamespace() : string {
		return ($this->getPlugin()
			->isDara()) ? DARA_XMLNS : DATACITE_XMLNS;
	}
	...
	private function getSchemaLocation() : string {
		return ($this->getPlugin()
			->isDara()) ? DARA_XSI_SCHEMA_LOCATION : DATACITE_XSI_SCHEMA_LOCATION;
	}
	...
	private function createResourceIdentifier($documentNode, $object) {
		...
		if (isset($pubId) && $this->getPlugin()
				->isTestMode($press)) {
			$pubId = preg_replace(
				'/^[\d]+(.)[\d]+/',
				$this->getPlugin()
					->getSetting($press->getId(), 'testPrefix'),
				$pubId
			);
		}
	...
	private function createDataURLs($documentNode, $object) {
		...
		$press = $request->getContext();
		if (NULL !== $press) {
			$testUrl = $this->getPlugin()
				->getSetting($press->getId(), 'testUrl');
			$host = ($this->getPlugin()
				->isTestMode($press)) ? $testUrl : $request->url($press->getPath());
		...
		foreach ($chapters as $chapter) {
			...
			if (isset($pubId)) {
				...
				if ($this->getPlugin()
					->isTestMode($press)) {
					$pubId = preg_replace(
						'/^[\d]+(.)[\d]+/', $this - $this->getPlugin()
											  ->getDataciteAPITestPrefix(), $pubId
					);
				}
